...
[0m[[32msuccess[0m] [0mTotal time: 0 s, completed Jun 21, 2013 10:16:20 PM[0m
[0m[[0minfo[0m] [0mLoading global plugins from /home/julianpeeters/.sbt/plugins[0m
[0m[[0minfo[0m] [0mSet current project to default-22f0e6 (in build file:/home/julianpeeters/asmifier-caseclass-experiments/)[0m
...
[0m[[32msuccess[0m] [0mTotal time: 14 s, completed Jun 21, 2013 10:16:39 PM[0m
import java.util.*;
...
mv.visitMethodInsn(INVOKESPECIAL, "MyRecord", "gd1$1", "(F)Z");
Label l2 = new Label();
...
mv.visitVarInsn(ALOAD, 0);
mv.visitMethodInsn(INVOKEVIRTUAL, "MyRecord", "x", "()F");
mv.visitMethodInsn(INVOKESTATIC, "scala/runtime/BoxesRunTime", "boxToFloat", "(F)Ljava/lang/Float;");
mv.visitInsn(ARETURN);
mv.visitLabel(l0);
...
{
mv = cw.visitMethod(ACC_PRIVATE + ACC_FINAL, "gd1$1", "(F)Z", null, null);
mv.visitCode();
mv.visitVarInsn(FLOAD, 1);
mv.visitVarInsn(ALOAD, 0);
mv.visitMethodInsn(INVOKEVIRTUAL, "MyRecord", "x", "()F");
...
{
mv = cw.visitMethod(ACC_PUBLIC, "<init>", "(F)V", null, null);
mv.visitCode();
mv.visitVarInsn(ALOAD, 0);
mv.visitVarInsn(FLOAD, 1);
